...
func (m *mailboxImpl) Initialize(dispatcher Dispatcher, handler MailboxHandler) {
...
func (m *mailboxImpl) HandleSystemMessage(message Message) {
	m.systemQueue.Push(unsafe.Pointer(&message))
	atomic.AddInt32(&m.sysNum, 1)
	m.dispatch()
}

func (m *mailboxImpl) HandleUserMessage(message Message) {
