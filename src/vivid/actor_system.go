...
type ActorSystem interface {
	...
	// PoisonStop 优雅停止 ActorSystem，
	// 返回可能发生的错误，如果停止成功则返回 nil，
	// 在调用此方法后，系统将停止处理新消息，并尝试立即关闭所有 Actor。
	PoisonStop() error

	// PoisonStopP 优雅停止 ActorSystem，并在停止失败时引发 panic，
	// 这是一个便捷方法，适用于确信停止不会失败或希望在停止失败时立即终止程序的场景。
	PoisonStopP()
...
func NewActorSystem(configurator ...ActorSystemConfigurator) ActorSystem {
	...
	// 如果有全局监控配置，设置到系统级
	if config.globalMonitoring != nil {
		sys.globalMonitoring = config.globalMonitoring
		// 将外部监控接口包装为内部适配器，然后设置到底层系统中
		adapter := &globalMonitoringAdapter{metrics: config.globalMonitoring}
		sys.system.SetGlobalMonitoring(adapter)
	}
...
func (a *actorSystem) PoisonStop() error {
	return a.system.PoisonStop()
}

func (a *actorSystem) PoisonStopP() {
	if err := a.PoisonStop(); err != nil {
		panic(err)
	}
}
