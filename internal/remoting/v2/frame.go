...
var (
	errFrameTooLarge        = errors.New("remoting: frame data length exceeds limit")
	errFrameControlTooLarge = errors.New("remoting: frame control length exceeds limit")
	errFrameShortWrite      = errors.New("remoting: short frame write")
)
...
func dataFrameBytes(data []byte) []byte {
	return NewDataFrame(data).Bytes()
}
...
func readFrame(reader io.Reader, header []byte) (ctrlType uint32, ctrlData, data []byte, err error) {
	frame, err := ReadFrame(reader, header, FrameReadLimits{})
	if err != nil {
		return 0, nil, nil, err
	}
	return uint32(frame.Type), frame.ControlData, frame.Data, nil
}

func readFrameWithMaxDataLen(reader io.Reader, header []byte, maxDataLen uint32) (ctrlType uint32, ctrlData, data []byte, err error) {
	return readFrameWithLimits(reader, header, 0, maxDataLen)
}

func readFrameWithLimits(reader io.Reader, header []byte, maxCtrlLen, maxDataLen uint32) (ctrlType uint32, ctrlData, data []byte, err error) {
	frame, err := ReadFrame(reader, header, FrameReadLimits{
		MaxControlLen: maxCtrlLen,
