...
func (c *ActorConfiguration) WithActorLogger(logger log.Logger) *ActorConfiguration {
    c.Logger = logger
    return c
}
...
func WithActorLogger(logger log.Logger) ActorOption {
    return func(c *ActorConfiguration) {
        c.WithActorLogger(logger)
    }
}

// WithActorMailboxProvider 设置 Actor 的邮箱提供器
func (c *ActorConfiguration) WithActorMailboxProvider(provider mailbox.Provider) *ActorConfiguration {
...
func WithActorMailboxProvider(provider mailbox.Provider) ActorOption {
    return func(c *ActorConfiguration) {
        c.WithActorMailboxProvider(provider)
    }
}

// WithActorDispatcherProvider 设置 Actor 的消息调度器提供器
func (c *ActorConfiguration) WithActorDispatcherProvider(provider mailbox.DispatcherProvider) *ActorConfiguration {
