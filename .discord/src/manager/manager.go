...
var _ vivid2.Manager = (*manager)(nil)

type manager struct {
	options   vivid2.ManagerOptionsFetcher                // 管理器选项
	host      resource.Host                               // 主机地址
	root      vivid2.Process                              // 根进程
	processes *xsync.MapOf[resource.Path, vivid2.Process] // 用于存储所有进程的映射表
	server    vivid2.ManagerConnRegister                  // 服务器连接注册表
}
...
func (mgr *manager) RegisterProcess(process vivid2.Process) (id vivid2.ID, exist bool) {
...
func (mgr *manager) UnregisterProcess(operator, id vivid2.ID) {
	if id == nil {
		panic(fmt.Errorf("process id is nil"))
	}
...
// GetProcess 获取一个进程
func (mgr *manager) GetProcess(id vivid2.ID) (process vivid2.Process) {
	if id == nil {
		return mgr.root
	}
...
func (mgr *manager) initServer() error {
	...
	go func(launcher vivid2.ServerLauncher) {
