...
- **监控与审计**：订阅系统事件（如生命周期、死信），在 OnReceive 中记录或告警。死信事件类型为 **`ves.DeathLetterEvent`**，参见 [消息投递 - 死信](/docs/basics/message-delivery#死信)。启用集群后，可订阅 **`ves.ClusterMembersChangedEvent`**、**`ves.ClusterLeaderChangedEvent`** 做成员与主节点变更监控，参见 [集群事件](/docs/cluster/events/events)。
