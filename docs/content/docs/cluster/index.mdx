...
集群功能在启用 [远程通讯](/docs/config/remoting) 的前提下，通过 **ActorSystemRemotingOptions.ClusterOptions** 开启。启用后，系统会创建 NodeActor（路径 `/cluster/node`），维护成员视图、故障检测与选主，并通过 **ClusterContext** 暴露给业务：获取成员、多数派状态及成员变更事件等。

## 架构概览

- **NodeActor**：集群核心 Actor，由框架在初始化时创建（名称如 `@cluster`），负责 Gossip 协议、成员发现、故障检测、选主与视图合并。
...
### 运行时与事件
- [ClusterContext](/docs/cluster/runtime/context) — 运行时 API：GetMembers、InQuorum、Leave
