...
本文用图示说明集群的部署形态、节点加入与发现、Gossip 同步、故障检测与优雅退出，便于与实现对齐并排查部署差异。
...
<Mermaid diagram={`sequenceDiagram
  ...
  alt 无 Seeds 或 本节点在 Seeds 中
    N->>N: bootstrapAsSeed(): 自举为种子
    N->>View: AddMember(self), 启动 Gossip/FailureDetection
  else 有 Seeds 且 本节点不在 Seeds 中
    N->>Seed: Ask(host:port/@cluster, JoinRequest)
    Seed->>Seed: handleJoinRequest: 校验白名单/认证/Quorum
    Seed->>N: JoinResponse(View)
    N->>N: MergeFromWithOptions(View), AddMember(self)
    N->>View: 启动 Gossip / FailureDetection
  end
`} title="加入流程：自举为种子 或 向种子 Join" />

<Mermaid diagram={`flowchart LR
  A[OnLaunch] --> B{Seeds 为空?}
  B -->|是| C[bootstrapAsSeed]
  B -->|否| D{本节点在 Seeds 中?}
  D -->|是| C
  D -->|否| E[tryJoinSeeds]
  E --> F{任一种子 Join 成功?}
  F -->|是| G[合并视图, 启动 Gossip/FD]
  F -->|否| H[指数退避, 调度 JoinRetryTick]
  H --> E
`} title="加入决策：是否自举为种子" />
...
<Mermaid diagram={`sequenceDiagram
  ...
  Note over A, B: 周期 GossipTick / GossipCrossDCTick
  A->>A: 选目标: GossipTargetSelector
  A->>VA: Snapshot()
  A->>B: Tell(B/@cluster, GossipMessage(View))
  B->>B: handleGossip: MergeFromWithOptions
  B->>VB: 若 changed: 发布事件, 可选继续扩散
...
<Mermaid diagram={`flowchart LR
  ...
  View --> FD[FailureDetector.RunDetection]
  Now --> FD
  FD --> Suspect["toSuspect\n(置为 Suspect)"]
  FD --> Remove["toRemove\n(从视图剔除)"]
  Suspect --> Publish["PublishViewChanged\n(可选 SuspectConfirmDuration)"]
  Remove --> Publish
  Publish --> Quorum["重新计算 Quorum / 发布 Leader 事件"]
`} title="故障检测：超时 → Suspect → 剔除 → 事件" />

- **FailureDetectionTimeout**：同 DC 成员超时未刷新 LastSeen 则先 Suspect（若配置了 **SuspectConfirmDuration**），再剔除。
- **CrossDCFailureDetectionTimeout**：对跨 DC 成员单独超时（未配置时内部使用同 DC 的 2 倍），减少跨 DC 延迟导致的误判。
...
<Mermaid diagram={`sequenceDiagram
  ...
  App->>NA: Tell(LeaveRequest)
  NA->>NA: handleLeaveRequest: 调度 LeaveBroadcastRound
  loop 每轮 LeaveBroadcastDelay
    NA->>Others: Tell(GossipMessage(View 不含本节点))
  end
  NA->>NA: ExitingReady: 状态 Exiting, Reply(LeaveAck)
...
<Mermaid diagram={`flowchart LR
  ...
  Schedule --> Round[LeaveBroadcastRound 1..N]
  Round --> Broadcast[向当前视图成员 Gossip 不含本节点的 View]
  Broadcast --> Next{还有下一轮?}
  Next -->|是| Schedule
  Next -->|否| Exiting[ExitingReady]
  Exiting --> Ack[Reply LeaveAck]
  Exiting --> Event[ClusterLeaveCompletedEvent]
`} title="Leave 多轮广播与退出" />
...
<Mermaid diagram={`stateDiagram-v2
  ...
  Leaving --> Exiting: 多轮广播完成, ExitingReady
  Exiting --> [*]: 发布 LeaveCompleted
  Joining --> Joining: 失败则指数退避重试
  Active --> Discovering: 失多数派时 tryQuorumRecovery
`} title="节点生命周期状态" />
...
| **种子与自举** | 无 Seeds 或本节点在 Seeds 中 → bootstrapAsSeed；否则 tryJoinSeeds，目标为 `seed_host:port/@cluster`。 |
| **Gossip** | 内部消息类型为 ClusterInternalMessageForGossip；GetViewRequest 用于拉取视图（如 quorum 恢复）。 |
| **多 DC** | SeedsByDC、CrossDCFailureDetectionTimeout、CrossDCDiscoveryInterval、MaxDiscoveryTargetsPerTickCrossDC 见 [多数据中心](/docs/cluster/topology/multi-datacenter) 与 [配置选项](/docs/cluster/config/options)。 |

若部署行为与上述图示不一致，可对照 [配置选项](/docs/cluster/config/options)、[与 Remoting 的关系](/docs/cluster/remoting) 及代码中的 `internal/cluster` 实现排查。
