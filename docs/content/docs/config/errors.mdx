...
| **ErrorRemotingHandshakeFailed** | 远程握手失败 |

### 集群

集群名称校验、加入控制及未启用时的调用。完整说明见 [集群错误](/docs/cluster/errors)。

| 变量 | 说明 |
|------|------|
| **ErrorClusterNameMismatch** | 集群名称不匹配 |
| **ErrorClusterDisabled** | 集群已禁用（未启用集群时调用 ClusterContext 方法会返回此错误） |
| **ErrorClusterNodeStatusMismatch** | 节点状态不匹配 |
| **ErrorClusterNotInQuorum** | 当前不在多数派 |
| **ErrorClusterJoinAuthFailed** | Join 认证失败 |
| **ErrorClusterJoinRateLimited** | Join 请求被限流 |
| **ErrorClusterProtocolVersionMismatch** | 集群协议版本不兼容 |
| **ErrorClusterJoinNotAllowed** | 地址或 DC 不在白名单 |
| **ErrorClusterAdminAuthFailed** | 管理操作 Token 无效 |
