...
远程节点的 ActorRef 常以字符串形式存储或从配置/服务发现获取。使用 **system.FindActorRef(actorRef string)** 可将字符串解析为 **ActorRef**，再用于 Tell/Ask。详见 [创建与生命周期 - 通过字符串解析 ActorRef](/docs/basics/lifecycle#通过字符串解析-actorref)。
